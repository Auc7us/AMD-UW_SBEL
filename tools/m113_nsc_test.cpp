...
// [headless test] VSG visualization removed so this links without the VSG libs.
...
// Contact formulation (NSC or SMC)  -- SWITCHED TO NSC FOR THIS TEST
...
// Per-side contact-force reporter: classifies each contact by whether it
// involves a LEFT-track or RIGHT-track shoe body.
class SideForceReporter : public chrono::ChContactContainer::ReportContactCallback {
  public:
    std::set<chrono::ChContactable*> left, right;
    double fL = 0, fR = 0, fOther = 0;
    int nL = 0, nR = 0, nOther = 0;
    int nShoeShoe = 0, nShoeOther = 0;  // shoe-vs-shoe vs shoe-vs-nonshoe
    void reset() { fL = fR = fOther = 0; nL = nR = nOther = 0; nShoeShoe = nShoeOther = 0; }
    virtual bool OnReportContact(const chrono::ChVector3d&, const chrono::ChVector3d&,
                                 const chrono::ChMatrix33<>&, double, double,
                                 const chrono::ChVector3d& react_forces, const chrono::ChVector3d&,
                                 chrono::ChContactable* A, chrono::ChContactable* B, int) override {
        double f = react_forces.Length();
        bool aShoe = left.count(A) || right.count(A);
        bool bShoe = left.count(B) || right.count(B);
        bool l = left.count(A) || left.count(B);
        bool r = right.count(A) || right.count(B);
        if (l) { fL += f; nL++; }
        else if (r) { fR += f; nR++; }
        else { fOther += f; nOther++; }
        if (aShoe && bShoe) nShoeShoe++;
        else if (aShoe || bShoe) nShoeOther++;
        return true;
    }
};
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if (a == "smc" || a == "SMC") contact_method = ChContactMethod::SMC;
        if (a == "nsc" || a == "NSC") contact_method = ChContactMethod::NSC;
        if (a.rfind("env=", 0) == 0) {
            double e = std::stod(a.substr(4));
            chrono::ChCollisionModel::SetDefaultSuggestedEnvelope(e);
            std::cout << "[override] collision envelope = " << e << std::endl;
        }
    }
    ...
    auto reporter = chrono_types::make_shared<SideForceReporter>();
    {
        auto tL = vehicle.GetTrackAssembly(LEFT);
        auto tR = vehicle.GetTrackAssembly(RIGHT);
        for (size_t i = 0; i < tL->GetNumTrackShoes(); ++i)
            reporter->left.insert(tL->GetTrackShoe(i)->GetShoeBody().get());
        for (size_t i = 0; i < tR->GetNumTrackShoes(); ++i)
            reporter->right.insert(tR->GetTrackShoe(i)->GetShoeBody().get());
    }
    ...
    // [headless test] visualization disabled.
    ...
    // [diag] optional: override iterative-VI solver max iterations via iters=N
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if (a.rfind("iters=", 0) == 0) {
            if (auto vi = std::dynamic_pointer_cast<chrono::ChIterativeSolverVI>(m113.GetSystem()->GetSolver())) {
                vi->SetMaxIterations(std::stoi(a.substr(6)));
                std::cout << "[override] BB max iterations = " << a.substr(6) << std::endl;
            }
        }
    }
    ...
    std::cout << "[headless] contact=" << (contact_method == ChContactMethod::NSC ? "NSC" : "SMC")
              << " step=" << step_size << " running to t_end=" << t_end << " s\n";
    ...
    while (true) {
        ...
        if (step_number % 100 == 0) {
            reporter->reset();
            m113.GetSystem()->GetContactContainer()->ReportAllContacts(reporter);
            auto v = m113.GetChassisBody()->GetPosDt();
            double zL = vehicle.GetTrackAssembly(LEFT)->GetIdler()->GetWheelBody()->GetPos().z();
            double zR = vehicle.GetTrackAssembly(RIGHT)->GetIdler()->GetWheelBody()->GetPos().z();
            static auto wall0 = std::chrono::high_resolution_clock::now();
            static double sim0 = time;
            auto now = std::chrono::high_resolution_clock::now();
            double wall = std::chrono::duration<double>(now - wall0).count();
            double rtf = (time - sim0) > 1e-9 ? wall / (time - sim0) : 0.0;
            std::cout << std::fixed << std::setprecision(3)
                      << "t=" << time << "  spd=" << v.Length()
                      << "  Ncontact=" << m113.GetSystem()->GetNumContacts()
                      << " | shoe-shoe=" << reporter->nShoeShoe
                      << " shoe-other=" << reporter->nShoeOther
                      << " | idlerZ L=" << zL << " R=" << zR
                      << "  RTF=" << std::setprecision(2) << rtf << "\n";
        }
